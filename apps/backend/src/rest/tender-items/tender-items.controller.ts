...
async function resolveCompanyId(req: Request, res: Response): Promise<string | null> {
  if (req.resolvedCompanyId) return req.resolvedCompanyId;
  const tenderId = String(req.params.tenderId);
  const companyId = await tendersRepo.resolveCompanyIdForTender(tenderId);
  if (!companyId) {
...
export const getAll = async (req: Request, res: Response, next: NextFunction): Promise<void> => {
  try {
    const companyId = await resolveCompanyId(req, res); if (!companyId) return;
